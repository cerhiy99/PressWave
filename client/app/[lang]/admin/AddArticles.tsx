...
import { categoriesInterface } from "@/app/interfaces/categoriesInterface";
import { getCategories } from "../../functions/categories";
import React, { useEffect, useState } from "react";
import { hastagsInterface } from "@/app/interfaces/hastagInterface";
import { getHastags } from "../../functions/hestegs";
import { Editor } from "@tinymce/tinymce-react";
import { addArticles } from "../../functions/article";
...
const AddArticles = ()=> {
  ...
  const [hashtag,setHashtag]=useState<hastagsInterface>()


  const categoriesSet=async()=>{
    const categoris=await getCategories("en");
    setCategories(categoris);
  }

  const hastagsSet=async()=>{
  ...
  return (
    <div className="add-articles">
      ...
      categories:
      {Array.isArray(categories)&&categories.slice(3).map((category) => (
      ...
      hastags:
      {Array.isArray(hashtag) &&  hashtag.map((hastag) => (
      ...
      <Editor
        value={description3}
        apiKey="t6okxmezjfhajn8bk23u3dkejv0oc9c1qhs7gmmh9qskcfdp"
        onEditorChange={(newText:string) => setdescription3(newText)}
      />
